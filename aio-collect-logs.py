...
class TaskLogCollector:
    ...
    def __init__(self, task_id, stages=None, failed_only=False, subtask_ids=None):
        self.task_id = task_id
        self.stages = stages
        self.failed_only = failed_only
        self.subtask_ids = set(subtask_ids) if subtask_ids else None
    ...
    FAILED_STATES = ('failed', 'failed_uncleaned')
    ...
    def apply_subtask_filter(self):
        """根据 --subtask / --failed-only 过滤要收集的子任务"""
        total = len(self.sub_tasks)

        # 指定子任务ID优先级最高
        if self.subtask_ids is not None:
            kept = [s for s in self.sub_tasks if s['id'] in self.subtask_ids]
            missing = self.subtask_ids - {s['id'] for s in self.sub_tasks}
            if missing:
                print("  [WARN] 指定的子任务不属于该任务: {}".format(
                    ', '.join(str(i) for i in sorted(missing))))
            self.sub_tasks = kept
            print("  过滤: 仅收集指定子任务 {}/{} 个".format(len(kept), total))

        elif self.failed_only:
            kept = [s for s in self.sub_tasks if s['state'] in self.FAILED_STATES]
            self.sub_tasks = kept
            print("  过滤: 仅收集失败子任务 {}/{} 个".format(len(kept), total))
...
def main():
    if len(sys.argv) >= 2 and sys.argv[1] in ('--help', '-h'):
        ...
        print("  --failed-only        只收集失败的子任务(failed/failed_uncleaned)")
        print("                       适合10个子任务只有1个报错时，只取出错的那个")
        print("  --subtask <ids>      只收集指定子任务ID，逗号分隔")
    ...
    if '--subtask' in sys.argv:
        idx = sys.argv.index('--subtask')
        if idx + 1 < len(sys.argv):
            try:
                subtask_ids = [int(s.strip()) for s in sys.argv[idx + 1].split(',') if s.strip()]
            except ValueError:
                print("[ERROR] --subtask 参数必须是数字(可逗号分隔)")
                sys.exit(1)

    if failed_only and subtask_ids:
        print("[ERROR] --failed-only 和 --subtask 不能同时使用")
        sys.exit(1)

    try:
        load_config()
        collector = TaskLogCollector(task_id, stages=stages,
                                     failed_only=failed_only, subtask_ids=subtask_ids)
        collector.collect()
    finally:
