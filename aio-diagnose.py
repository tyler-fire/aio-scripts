...
class TaskDiagnostic:
    ...
    def collect_service_logs(self):
        ...
        service_logs = ["cdm.log", "scheduler.log", "worker.log", "apscheduler.log"]
        start_str = self.time_window['start'].strftime("%Y-%m-%d %H:%M:%S")
        end_str = self.time_window['end'].strftime("%Y-%m-%d %H:%M:%S")

        for log_name in service_logs:
            log_path = "{}/logs/service/{}".format(AIO_HOME, log_name)
            if not os.path.exists(log_path):
                continue

            # 用 awk 提取时间窗口内的日志（简单按行匹配）
            output_file = os.path.join(service_dir, log_name)
            try:
                # 使用 grep 提取关键词行
                cmd = "grep -E '{}' {} 2>/dev/null | tail -1000 > {}".format(
                    '|'.join(SYSTEM_LOG_KEYWORDS), log_path, output_file
                )
                subprocess.run(cmd, shell=True, timeout=30)
                if os.path.exists(output_file) and os.path.getsize(output_file) > 0:
                    print("  ✓ {} ({:.1f} KB)".format(log_name, os.path.getsize(output_file)/1024))
                else:
    ...
    def collect_system_logs(self):
        ...
        if os.path.exists("/var/log/messages"):
            try:
                cmd = "grep -E '{}' /var/log/messages 2>/dev/null | tail -1000 > {}".format(
                    '|'.join(SYSTEM_LOG_KEYWORDS),
                    os.path.join(system_dir, "messages.log")
                )
                subprocess.run(cmd, shell=True, timeout=60)
                msg_file = os.path.join(system_dir, "messages.log")
                if os.path.exists(msg_file) and os.path.getsize(msg_file) > 0:
                    print("  ✓ messages.log ({:.1f} KB)".format(os.path.getsize(msg_file)/1024))
                else:
